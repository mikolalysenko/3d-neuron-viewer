...
function clipCube (worldToClip) {
  ...
  for (i = 0; i < CUBE_PLANES.length; ++i) {
...
proto.draw = function () {
  ...
  shader.uniforms.stepSize = this.step
  shader.uniforms.transparency = this.transparency
  shader.uniforms.intensity = this.intensity
...
function createVolumeRenderer (gl, array) {
  ...
  var s0 = ndarray(data,
    [zsplit[1], array.shape[1] + 2, zsplit[0], array.shape[2] + 2],
    [gridW * (array.shape[1] + 2), gridW, (array.shape[2] + 2), 1],
    0)
  ...
  for (i = 0; i < nx; ++i) {
    ...
    ops.assign(s0
      .pick(t, -1, s, -1)
      .lo(1, 1)
      .hi(array.shape[1], array.shape[2]),
    array.pick(i))
  }
